...
const DON_CSS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;font-family:'Times New Roman',Times,serif}
  body{font-size:14pt;line-height:1.5;color:#000;background:#e0e0e0;display:flex;justify-content:center;padding:20px}
  .page{background:#fff;width:210mm;min-height:297mm;padding:20mm 15mm 20mm 30mm;box-shadow:0 5px 15px rgba(0,0,0,0.2);box-sizing:border-box}
  @media print { body{background:#fff;padding:0;display:block;margin:0} .page{width:100%;height:100%;padding:20mm 15mm 20mm 30mm!important;margin:0;box-shadow:none} }
...
document.getElementById('btnPrint').onclick = async () => {
  ...
  const html = `<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8">
    ...
    <style>${DON_CSS}</style></head><body><div class="page">${buildDonHTML(d)}</div>
...
async function expDocx(d, fn) {
  ...
  function para(a,o){o=o||{};return'<w:p><w:pPr><w:jc w:val="'+(o.jc||'both')+'"/>'+(o.ind?'<w:ind w:firstLine="709"/>':'')+'<w:spacing w:after="0" w:line="360" w:lineRule="auto"/></w:pPr>'+a.map(r=>run(r.t,r)).join('')+'</w:p>';}
  ...
  if(!has2){
    sig='<w:tbl><w:tblPr><w:tblW w:w="'+TW+'" w:type="dxa"/>'+nb+'</w:tblPr><w:tblGrid><w:gridCol w:w="'+W+'"/><w:gridCol w:w="'+WM+'"/><w:gridCol w:w="'+W+'"/></w:tblGrid>'
      +'<w:tr>'+cell(cp([{t:'Ng\u01b0\u1eddi \u0111\u1ed5i'}]),W)+cell(cp([{t:''}]),WM)+cell(cp([{t:'Ng\u01b0\u1eddi vi\u1ebft \u0111\u01a1n'}]),W)+'</w:tr>'
      +emptyRow3+emptyRow3+emptyRow3+emptyRow3+emptyRow3
      +'<w:tr>'+cell(cp([{t:cl(d.nd)}]),W)+cell(cp([{t:''}]),WM)+cell(cp([{t:cl(d.nx)}]),W)+'</w:tr>'
      +'</w:tbl>'
      +'<w:p><w:pPr><w:spacing w:after="0"/></w:pPr></w:p>'
      +'<w:tbl><w:tblPr><w:tblW w:w="'+TW+'" w:type="dxa"/>'+nb+'</w:tblPr><w:tblGrid><w:gridCol w:w="'+TW+'"/></w:tblGrid>'
      +'<w:tr>'+cell(cp([{t:'\u0110\u1ed9i tr\u01b0\u1edfng',bold:true}]),TW)+'</w:tr>'
...
function buildContext() {
  ...
  return `Bạn là trợ lý AI của hệ thống Quản lý Đổi Ca Sở Xây Dựng TP.HCM.
